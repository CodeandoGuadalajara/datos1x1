...
global $gpp_theme_options;

$gpp_theme_options = array();

/**
* GPP Theme Options Version
*/
define( 'GPP_THEME_OPTIONS_VER', '1.4.2' );

/**
* Set some default theme options when theme is switched to this theme
*/
function gpp_theme_options_activation_func() {

    $defaults = (array) gpp_get_options();

    if ( ! get_option( gpp_get_current_theme_id() . "_options" ) )
        update_option( gpp_get_current_theme_id() . "_options" , $defaults );

}
add_action( 'after_switch_theme', 'gpp_theme_options_activation_func', 10, 2 );

/**
* Merge existing and new option arrays
*/
function gpp_register_theme_options( $options ) {
    global $gpp_theme_options;
    $gpp_theme_options = array_merge( $gpp_theme_options, $options );
}

/**
* Extract tabs array
*/
function gpp_register_theme_option_tab( $args ) {
    global $gpp_tabs;
    extract( $args );
    if ( $name ) :
        $gpp_tabs[] = $args;
 ...
 */
function gpp_get_current_theme_id() {
 ...
 */
function gpp_get_theme_options_directory_uri() {
...
*/
function gpp_after_setup_theme() {

    include_once( 'library/theme-customizer.php' );

}
add_action( 'after_setup_theme', 'gpp_after_setup_theme', 10 );

/**
* Enqueue CSS and Javascripts
*/
function gpp_enqueue_scripts_styles() {

    wp_enqueue_style( 'gpp-framework', gpp_get_theme_options_directory_uri() . 'css/gpp-framework.css' );
    wp_enqueue_script( 'gpp-framework', gpp_get_theme_options_directory_uri() . 'js/gpp-framework.js', array( 'jquery' ) );
...
add_action( 'admin_print_scripts-appearance_page_gpp-settings', 'gpp_enqueue_scripts_styles', 40 );

/**
* Settings API options initilization and validation
*/
function gpp_register_options() {
	global $wp_customize;
...
add_action( 'admin_init', 'gpp_register_options' );

/**
* Settings API actions initilization and validation
*/
function gpp_register_actions() {
    include_once( 'library/actions.php' );
}
add_action( 'init', 'gpp_register_actions' );
 ...
 */
function gpp_add_theme_page() {
    // Globalize Theme options page
    global $gpp_settings_page;
    // Add Theme options page
    $gpp_settings_page = add_theme_page(
        // $page_title
        ...
        gpp_get_settings_page_cap(),
        // $menu_slug
        // String to append to URL after "themes.php"
        'gpp-settings',
        // $callback
        // Function to define settings page markup
        'gpp_admin_options_page'
    );
}
add_action( 'admin_menu', 'gpp_add_theme_page' );
...
function gpp_admin_options_page() {
    ...
    <div class="wrap">
        ...
        <form action="options.php" method="post">
            ...
            ?>
            <?php submit_button( __( 'Save Settings', 'gpp' ), 'primary', gpp_get_current_theme_id() . "_options[submit-{$currenttab}]", false ); ?>
            <?php submit_button( __( 'Reset Defaults', 'gpp' ), 'secondary', gpp_get_current_theme_id() . "_options[reset-{$currenttab}]", false ); ?>
        </form>
    </div>
<?php
}
 ...
 */
function gpp_get_option_defaults() {
 ...
 */
function gpp_get_option_default( $name ) {
    // Get the array that holds all
    // Theme option parameters
    $option_parameters = gpp_get_option_parameters();
    // Initialize the array to hold
 ...
 */
function gpp_get_option_parameters() {

    global $gpp_theme_options;
    return $gpp_theme_options;
...
function gpp_get_settings_page_cap() {
    return 'edit_theme_options';
}
// Hook into option_page_capability_{option_page}
add_action( 'option_page_capability_gpp-settings', 'gpp_get_settings_page_cap' );
...
function gpp_field_text( $value, $attr ) { ?>
    <input type="text" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" value="<?php echo esc_attr( $value ); ?>">
<?php
}

function gpp_field_hidden( $value, $attr ) { ?>
    <input type="hidden" class="<?php echo $attr['name']; ?>" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" value="<?php echo esc_attr( $value ); ?>">
<?php
}

function gpp_field_textarea( $value, $attr ) { ?>
    <textarea name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" cols="48" rows="8"><?php echo stripslashes_deep( $value ); ?></textarea>
<?php
}

function gpp_field_select( $value, $attr ) { ?>
<select name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]">
    <?php
...
function gpp_field_radio_image( $value, $attr ) { ?>
    <?php
    if ( isset( $attr['valid_options'] ) ) :
        $options = $attr['valid_options'];
        foreach( $options as $option ) :
    ...
    <input type="radio" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" value="<?php echo $option['name']; ?>" <?php checked($option['name'], $value ); ?>>
...
function gpp_field_radio( $value, $attr ) { ?>
    <?php
    if ( isset( $attr['valid_options'] ) ) :
        $options = $attr['valid_options'];
        foreach( $options as $option ) :
        ?>
    <label class="radio">
      <input type="radio" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" value="<?php echo $option['name']; ?>" <?php checked( $option['name'], $value ); ?>> <?php echo $option['title']; ?>
    </label>
            <?php
        endforeach;
    else:
...
function gpp_field_checkbox( $value, $attr ) {

    if ( isset( $attr['valid_options'] ) ) :
        $options = $attr['valid_options'];

        foreach( $options as $option_key => $option_value ) : ?>
            <input class="checkbox" id ="<?php echo $attr['name']; ?>" type="checkbox" <?php if( isset( $value ) && '' != $value ) { checked( in_array( $option_value['name'], $value ) ); }  ?> name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>][]" value="<?php echo esc_attr( $option_key ); ?>">
...
function gpp_field_color( $value, $attr ) { ?>
    <?php $defaultcolor = $attr['default']; ?>
    <span class="colorPickerWrapper">
        <input id="<?php echo $attr['name']; ?>" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" class="color-picker" type="text" data-default-color="<?php echo $defaultcolor; ?>" value="<?php echo $value; ?>" />
    </span>

<?php
}

function gpp_field_image( $value, $attr ) { ?>
    ...
    <div id="<?php echo $attr['name']; ?>_container">
        <input type="text" class="upload_image_field" id="<?php echo $attr['name']; ?>" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" value="<?php echo $value; ?>">
...
function gpp_field_gallery( $value, $attr ) {
    ...
    jQuery(document).ready(function() {
        ...
        $gcontainer = jQuery("#<?php echo $attr['name']; ?>_container");
        $image_field = $gcontainer.find('.upload_gallery_field');
        $image_button = $gcontainer.find('.upload_gallery_button');
        $remove_button = $gcontainer.find('.gpp_gallery_remove');
        ...
        $image_button.click(function() {
            ...
            file_frame.on( 'select', function() {
                ...
                selection.map( function( attachment ) {
                    ...
                    $gcontainer.find( '.upload_gallery_preview' ).show();
                ...
                $gcontainer.find( '.upload_gallery_field' ).val( hiddenids.substring( 0, hiddenids.length - 1 ) ).show();
            });
    ...
    <div id="<?php echo $attr['name']; ?>_container">
        <input type="hidden" class="upload_gallery_field" id="<?php echo $attr['name']; ?>" name="<?php echo gpp_get_current_theme_id(); ?>_options[<?php echo $attr['name']; ?>]" value="<?php echo $value; ?>">
 ...
 */
function gpp_image_url_callback() {
    $ids = $_POST['ids'];
    $pid = $_POST['pid'];
...
add_action( 'wp_ajax_gpp_imageurl', 'gpp_image_url_callback' );

/**
 * Custom Font Previews
 */
function gpp_fonts_preview() {
...
add_filter( "attachment_fields_to_edit", "gpp_image_attachment_fields_to_edit", null, 2 ); 
function gpp_image_attachment_fields_to_edit( $form_fields, $post ) {	
...
add_filter("attachment_fields_to_save", "gpp_image_attachment_fields_to_save", null, 2);
function gpp_image_attachment_fields_to_save( $post, $attachment ) {	
